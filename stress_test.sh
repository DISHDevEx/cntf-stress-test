...
for _ in {1..10}; do
    ...
    $ping_command | jq -c '. + {"test": "stress_test"}' | tee -a stress_test_logs.txt || 2>> ./stress_test_error_logs.txt # this outputs the logs from each ping to "stress_test_logs.txt" and any error logs to "stress_test_error_logs.txt"
